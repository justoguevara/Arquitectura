...
function decidifica_estilos(caracteristicas){
	...
	for(var i=0; i<caracteristicas.length; i++){
		...
		if( tipo == "borsegmento" ){ cadena += "border-style:;"}   // segmentado
		if( tipo == "borpunteado" ){ cadena += "border-style:;"}   // punteado
		if( tipo == "bordoble" ){ cadena += "border-style:; "}      // doble
...
function estilos(elemento,ess){  // dimencionado
	veri = valid_add(ess.id);
	id = veri.id;
	pref = "es_";     // pref: prefijo clase es
	pr_sel = "sel_";  // pr_sel: prefijo clase selec
	if(veri.existe==false){
		if(typeof(ess.es)=="string"){
			var cadena = decidifica_estilos(ess.es);
	...
	if(ess.marc){
		return " class='" +  pr_sel + id + "'"; // retorna cadena de selec
	}else{
		return " class='" +  pref + id + "'"; // retorna cadena de es
	}
}
